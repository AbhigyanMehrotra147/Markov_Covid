...
class Global_Dynamic_Mean(SN):
    ...
    def Divide_by_max_and_add(self, frame_size):
        ...
        for i in range(1, row_size):
            ...
            if i > row_size-frame_size:
                ...
                continue
            global_dynamic_max = temp_data_frame[i:i + frame_size+1].max().max()
            self.Dataframe_with_countries_as_column.iloc[i:i+frame_size + 1] = temp_data_frame[i:i+frame_size+1].applymap(lambda x: x/global_dynamic_max)
    ...
    def Divide_by_frame_size(self, frame_size):
        ...
        #condition for the first frame data points
