...
class Super_Normalization():
    ...
    # Countries seperated by giving half weightage to populations and half to density
    weighted_population_and_density_in_ascending_order = SCPD.weighted_density_and_population_in_order
    ...
    def divide_by_population(self, DataFrame=None):
        for country in DataFrame:
            if country in self.population_and_density_df:
    ...
    def get_final_df_Dictionary(self, rolling_days=14):
        ...
        for parameter in col_array:
            ...
            interpolated_df = deleted_nan_country_df.interpolate(
                limit_area="inside")
            # # Dividing Each Country's data with it's own population
            # for country in interpolated_df:
            #     if country not in self.population_and_density_df.index:
            #         continue

            #     interpolated_df[country] = interpolated_df[country].apply(
            #         lambda x: x/self.population_and_density_df.loc[country, "pop"])
            self.divide_by_population(interpolated_df)
            interpolateddf = self.rolling_average(
    ...
    @staticmethod
    def segregate_countries(DataFrame_to_be_segregated, first_threshod=0.05, second_threshold=0.2, third_threshold=1):
        Dataframe = DataFrame_to_be_segregated.copy(deep=True)
        c1 = []
        c2 = []
        c3 = []
        for column in Dataframe.columns:
            Max = Dataframe[column].max()
            if Max < first_threshod:
